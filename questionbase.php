 ...
 * - question_graded_automatically_with_countback implements
 ...
 * - question_first_matching_answer_grading_strategy implements
...
abstract class question_definition {
	...
	/**
	 *
	 * @var boolean whethre this question has been deleted/hidden in the question
	 *      bank.
	 */
	public $hidden = 0;
	...
	/**
	 * Constructor.
	 * Normally to get a question, you call
	 ...
	 * what is asked for.
	 *
	 * If your question wants to do somthing especially complicated is some
	 * situations,
	 ...
	 *        	the attempt we are creating a behaviour for.
	 * @param string $preferredbehaviour
	 *        	the requested type of behaviour.
	 ...
	 *
	 * @param
	 *        	question_attempt_step The first step of the {@link question_attempt}
	 ...
	 * inappropriate.
	 *
	 * @return string null plain text summary of this question.
	 */
	 ...
	 *
	 * @return array string name => PARAM_... constant, or, as a special case
	 *         that should only be used in unavoidable, the constant
	 *         question_attempt::USE_RAW_DATA
	 ...
	 *
	 * @return array null name => value.
	 */
	public abstract function get_correct_response();
	
	/**
	 * Takes an array of values representing a student response represented in a way
	 * that is understandable by a human and
	 * transforms that to the response as the POST values returned from the HTML form
	 * that takes the student response during a
	 * student attempt.
	 * Primarily this is used when reading csv values from a file of student responses
	 * in order to be able to
	 * simulate the student interaction with a quiz.
	 *
	 * In most cases the array will just be returned as is. Some question types will
	 * need to transform the keys of the array,
	 * as the meaning of the keys in the html form is deliberately obfuscated so that
	 * someone looking at the html does not get an
	 * advantage. The values that represent the response might also be changed in
	 * order to more meaningful to a human.
	 *
	 * See the examples of question types that have overridden this in core and also
	 * see the csv files of simulated student
	 ...
	 *
	 * Also see {@link https://github.com/jamiepratt/moodle-quiz_simulate}, a quiz
	 * report plug in for uploading and downloading
	 * student responses as csv files.
	 *
	 * @param array $simulatedresponse
	 *        	an array of data representing a student response
	 * @return array a response array as would be returned from the html form (but
	 *         without prefixes)
	 */
	 ...
	 *
	 * This takes a student response (the POST values returned from the HTML form that
	 * takes the student response during a
	 * student attempt) it then represents it in a way that is understandable by a
	 * human.
	 *
	 * Primarily this is used when creating a file of csv from real student responses
	 * in order later to be able to
	 * simulate the same student interaction with a quiz later.
	 *
	 * @param string[] $realresponse
	 *        	the response array as was returned from the form during a student
	 *        	attempt (without prefixes).
	 ...
	 *
	 * @param string $text
	 *        	some content that needs to be output.
	 * @param int $format
	 *        	the FORMAT_... constant.
	 * @param question_attempt $qa
	 *        	the question attempt.
	 * @param string $component
	 *        	used for rewriting file area URLs.
	 * @param string $filearea
	 *        	used for rewriting file area URLs.
	 * @param bool $clean
	 *        	Whether the HTML needs to be cleaned. Generally,
	 *        	parts of the question do not need to be cleaned, and student input
	 *        	does.
	 ...
	 *
	 * @param string $text
	 *        	The HTML to reduce to plain text.
	 * @param int $format
	 *        	the FORMAT_... constant.
	...
	public function format_generalfeedback($qa) {
		return $this->format_text ( $this->generalfeedback, 
				$this->generalfeedbackformat, $qa, 'question', 'generalfeedback', 
				$this->id );
	}
	 ...
	 *
	 * @param question_attempt $qa
	 *        	the question attempt being displayed.
	 * @param question_display_options $options
	 *        	the options that control display of the question.
	 * @param string $component
	 *        	the name of the component we are serving files for.
	 * @param string $filearea
	 *        	the name of the file area.
	 * @param array $args
	 *        	the remaining bits of the file path.
...
class question_information_item extends question_definition {
	...
	public function make_behaviour(question_attempt $qa, $preferredbehaviour) {
		return question_engine::make_behaviour ( 'informationitem', $qa, 
				$preferredbehaviour );
	}
...
interface question_manually_gradable {
	 ...
	 *        	the responses previously recorded for this question,
	 *        	as returned by {@link question_attempt_step::get_qt_data()}
	 * @param array $newresponse
	 *        	the new responses, in the same format.
	 ...
	 *
	 * @param $response a
	 *        	response, as might be passed to {@link grade_response()}.
	 * @return string a plain text summary of that response, that could be used in
	 *         reports.
	 */
...
class question_classified_response {
	...
	public static function no_response() {
		return new question_classified_response ( null, 
				get_string ( 'noresponse', 'question' ), null );
	}
}
...
interface question_automatically_gradable extends question_manually_gradable {
	 ...
	 * get_min_fraction() and get_max_fraction(), and the corresponding {@link
	 * question_state}
	 ...
	 * The question_attempt is passed in case
	 ...
	 *
	 * @param int $hintnumber
	 *        	Which hint to display. Indexed starting from 0
	 * @param question_attempt $qa
	 *        	The question_attempt.
	 */
	 ...
	 * inappropriate.
	 *
	 * @return string null plain text summary of the right answer to this question.
	 */
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
interface question_automatically_gradable_with_countback extends 
	 ...
	 *        	the response for each try. Each element of this
	 *        	array is a response array, as would be passed to {@link
	 *        	grade_response()}.
...
/**
 * This class represents a real question.
 * That is, one that is not a
 * {@link question_information_item}.
 *
 * @copyright 2009 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class question_with_responses extends question_definition implements 
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class question_graded_automatically extends question_with_responses implements 
		question_automatically_gradable {
	/**
	 *
	 * @var Some question types have the option to show the number of sub-parts
	 *      correct.
	 */
	...
	protected function check_combined_feedback_file_access($qa, $options, $filearea) {
		...
		return $options->feedback &&
				 $state->get_feedback_class () . 'feedback' == $filearea;
	}
	...
	public function format_hint(question_hint $hint, question_attempt $qa) {
		return $this->format_text ( $hint->hint, $hint->hintformat, $qa, 'question', 
				'hint', $hint->id );
	}
}
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class question_graded_automatically_with_countback extends question_graded_automatically implements 
		question_automatically_gradable_with_countback {
	public function make_behaviour(question_attempt $qa, $preferredbehaviour) {
		if ($preferredbehaviour == 'interactive') {
			return question_engine::make_behaviour ( 'interactivecountback', $qa, 
					$preferredbehaviour );
		}
		return question_engine::make_archetypal_behaviour ( $preferredbehaviour, $qa );
	}
...
abstract class question_graded_by_strategy extends question_graded_automatically {
	...
	/**
	 *
	 * @param question_grading_strategy $strategy
	 *        	the strategy to use for grading.
	 */
	...
	public function classify_response(array $response) {
		if (empty ( $response ['answer'] )) {
			return array (
					$this->id => question_classified_response::no_response () 
			);
		}
		...
		if (! $ans) {
			return array (
					$this->id => new question_classified_response ( 0, 
							$response ['answer'], 0 ) 
			);
		}
		
		return array (
				$this->id => new question_classified_response ( $ans->id, 
						$response ['answer'], $ans->fraction ) 
		);
	}
}
...
class question_answer {
	...
	/**
	 *
	 * @var integer one of the FORMAT_...
	 *      constans.
	 */
...
class question_hint {
	...
	/**
	 *
	 * @var integer The corresponding text FORMAT_...
	 *      type.
	 */
	public $hintformat;
	...
	/**
	 * Create a basic hint from a row loaded from the question_hints table in the
	 * database.
	 *
	 * @param object $row
	 *        	with $row->hint set.
...
class question_hint_with_parts extends question_hint {
	/**
	 *
	 * @var boolean option to show the number of sub-parts of the question that were
	 *      right.
	 */
	...
	/**
	 *
	 * @var boolean option to clear the parts of the question that were wrong on
	 *      retry.
	 */
	public $clearwrong;
	 ...
	 * @param
	 *        	int the corresponding text FORMAT_... type.
	 * @param bool $shownumcorrect
	 *        	whether the number of right parts should be shown
	 * @param bool $clearwrong
	 *        	whether the wrong parts should be reset.
	 */
	 ...
	 *
	 * @param object $row
	 *        	with $row->hint, ->shownumcorrect and ->clearwrong set.
 ...
 * Used to share grading code between
...
interface question_response_answer_comparer {
	 ...
	 */
	public function compare_response_with_answer(array $response, 
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class question_first_matching_answer_grading_strategy implements 
		question_grading_strategy {
	/**
	 *
	...
	/**
	 *
	 * @param question_response_answer_comparer $question
	 *        	(presumably also a
	 *        	{@link question_definition}) the question we are doing the grading
	 *        	for.
	 */
