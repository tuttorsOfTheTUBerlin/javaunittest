...
class qtype_javaunittest_question extends question_graded_automatically {
	...
	/**
	 * Call local or remote execute function.
	 * Evaluation of junit output is done.
	 * Grade is calculated and feedback generated.
	 *
	 * @param array $response
	...
	public function grade_response(array $response) {
		...
		if ($this->questionattemptid === null) {
			...
			throw new Exception('javaunittest: grade_response, no questionattemptid');
		}
		...
		if ($ret['error']) {
			...
			if (!$found) {
				...
				if (strpos($output, 'java.lang.StackOverflowError') !== false) {
					$feedback .= get_string('stackoverflow', 'qtype_javaunittest');
					$feedback .= "<br>\n";
				}

				if (strpos($output, 'java.lang.OutOfMemoryError') !== false) {
					$feedback .= get_string('outofmemory', 'qtype_javaunittest');
					$feedback .= "<br>\n";
				}
		...
		$cur_feedback = $DB->get_record('qtype_javaunittest_feedback',
				array('questionattemptid' => $this->questionattemptid));
	...
	function local_execute($response) {
		...
		$temp_folder = $CFG->dataroot . '/temp/javaunittest_tmp_' .
				$this->id . '_' . $this->questionattemptid;
		...
		try {
			...
			$command = $cfg_plugin->pathjava . ' -Xmx' . $cfg_plugin->memory_xmx .
					...
					$cfg_plugin->pathpolicy . ' -cp ' . $cfg_plugin->pathjunit . ':' . $cfg_plugin->pathhamcrest .
					':' . $temp_folder . ' org.junit.runner.JUnitCore ' . $this->testclassname;
			
			$output = '';
			$testruntime = 0;
			
			$ret_proc = open_process($cfg_plugin->precommand . '; exec ' . escapeshellcmd($command), $cfg_plugin->timeoutreal,
					$cfg_plugin->memory_limit_output * 1024, $output, $testruntime);
	...
	function remote_execute($response) {
		...
		curl_setopt($curlHandle, CURLOPT_USERPWD, $cfg_plugin->remoteserver_user . ':' . $cfg_plugin->remoteserver_password);
		$result = curl_exec($curlHandle);
		$HTTPStatusCode = curl_getinfo($curlHandle, CURLINFO_HTTP_CODE);
		curl_close($curlHandle);
		
		if ($HTTPStatusCode != 200) {
			return array('error' => true, 'errortype' => 'REMOTE_SERVER_ERROR', 'message' => $result);
		}
		
		$json = json_decode($result, true);
		if ($json === null) {
			return array('error' => true, 'errortype' => 'REMOTE_SERVER_ERROR', 'message' => 'JSON decoding error');
		}
	...
	function compile($file) {
		...
		$command = $cfg->pathjavac . ' -nowarn -cp ' . $cfg->pathjunit . ' -sourcepath ' . 
			dirname($file) . ' ' . $file;
	...
	function delTree($dir) {
		...
		foreach ( $files as $file ) {
			(is_dir ( "$dir/$file" )) ? $this->delTree ( "$dir/$file" ) : unlink ( 
					"$dir/$file" );
		}
		$rc = rmdir ( $dir );
		return $rc;
