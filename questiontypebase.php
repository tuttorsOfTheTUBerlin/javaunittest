...
class question_type {
	 ...
	 *         You should not need to override this method, the default behaviour
	 *         should be fine.
	 */
	 ...
	 * dropdown.
	 * Override this method to return false if you don't want your
	 ...
	 *
	 * @return mixed the desired string, or false to hide this question type in the
	 *         menu.
	 */
	 ...
	 * @param string $otherquestionsinuse
	 *        	comma-separate list of other question ids in this attempt.
	 * @return bool true if a particular instance of this question requires manual
	 *         grading.
	 */
	...
	/**
	 *
	 * @return bool true if this question type can be used by the random question
	 *         type.
	 */
	 ...
	 * you want the base class to automatically save, backup and restore the extra
	 * fields,
	 * override this method to return an array wherer the first element is the table
	 * name,
	...
	/**
	 * If you use extra_question_fields, overload this function to return question id
	 * field name
	 ...
	 *        	passed on to the constructor call.
	 * @return object an instance of the form definition, or null if one could not be
	 *         found.
	 */
	public function create_editing_form($submiturl, $question, $category, $contexts, 
			$formeditable) {
		global $CFG;
		require_once ($CFG->dirroot . '/question/type/edit_question_form.php');
		$definitionfile = $CFG->dirroot . '/question/type/' . $this->name () . '/edit_' .
				 $this->name () . '_form.php';
		...
		if (! class_exists ( $classname )) {
			throw new coding_exception ( 
					$this->plugin_name () . ' does not define the class ' .
							 $this->plugin_name () . '_edit_form.' );
		}
		return new $classname ( $submiturl, $question, $category, $contexts, 
				$formeditable );
	}
	...
	/**
	 * This method should be overriden if you want to include a special heading or
	 * some other
	 * html on a question editing page besides the question editing form.
	 *
	 * @param question_edit_form $mform
	 *        	a child of question_edit_form
	 * @param object $question        	
	...
	public function display_question_editing_page($mform, $question, $wizardnow) {
		...
		echo $OUTPUT->heading_with_help ( $heading, 'pluginname', 
				$this->plugin_name () );
	 ...
	 *        	the question data, loaded from the databsae,
	 *        	or more likely a newly created question object that is only
	 *        	partially
	 *        	initialised.
	 */
	 ...
	 * determined by testing !empty($question->id). If it is not empty, we are
	 * updating.
	 *
	 * The question will be saved in category $form->category.
	 *
	 * @param object $question
	 *        	the question object which should be updated. For a
	 *        	new question will be mostly empty.
	 * @param object $form
	 *        	the object containing the information to save, as if
	...
	public function save_question($question, $form) {
		...
		$question->generalfeedbackformat = ! empty ( 
				$form->generalfeedback ['format'] ) ? $form->generalfeedback ['format'] : 0;
		
		if (empty ( $question->name )) {
			$question->name = shorten_text ( 
					strip_tags ( $form->questiontext ['text'] ), 15 );
			if (empty ( $question->name )) {
				$question->name = '-';
			}
		}
		...
		if ($question->penalty > 1 or $question->penalty < 0) {
			$question->errors ['penalty'] = get_string ( 'invalidpenalty', 
					'question' );
		}
		...
		if (! empty ( $question->questiontext ) &&
				 ! empty ( $form->questiontext ['itemid'] )) {
			$question->questiontext = file_save_draft_area_files ( 
					$form->questiontext ['itemid'], $context->id, 'question', 
					'questiontext', ( int ) $question->id, $this->fileoptions, 
					$question->questiontext );
		}
		if (! empty ( $question->generalfeedback ) &&
				 ! empty ( $form->generalfeedback ['itemid'] )) {
			$question->generalfeedback = file_save_draft_area_files ( 
					$form->generalfeedback ['itemid'], $context->id, 'question', 
					'generalfeedback', ( int ) $question->id, $this->fileoptions, 
					$question->generalfeedback );
		}
	 ...
	 *
	 * This is called by {@link save_question()} to save the question-type specific
	 * data
	 *
	 * @return object $result->error or $result->notice
	 * @param object $question
	 *        	This holds the information from the editing form,
	...
	public function save_question_answers($question) {
		...
		// We need separate arrays for answers and extra answer data, so no JOINS
		// there.
		...
		if ($isextraanswerfields) {
			...
			if (! empty ( $oldanswers )) {
				$oldanswerextras = $DB->get_records_sql ( 
						"SELECT * FROM {{$extraanswertable}} WHERE " .
		...
		foreach ( $question->answer as $key => $answerdata ) {
			...
			if ($isextraanswerfields) {
				...
				if (! $answerextra) {
					...
					// Avoid looking for correct default for any possible DB field
					// type
					// by setting real values.
					$answerextra = $this->fill_extra_answer_fields ( $answerextra, 
							$question, $key, $context, $extraanswerfields );
					$answerextra->id = $DB->insert_record ( $extraanswertable, 
							$answerextra );
				} else {
		...
		foreach ( $oldanswers as $oldanswer ) {
			$fs->delete_area_files ( $context->id, 'question', 'answerfeedback', 
					$oldanswer->id );
	...
	/**
	 * Returns true is answer with the $key is empty in the question data and should
	 * not be saved in DB.
	 *
	 * The questions using question_answers table may want to overload this. Default
	 * code will work
	 * for shortanswer and similar question types.
	 *
	 * @param object $questiondata
	 *        	This holds the information from the question editing form or import.
	 * @param int $key
	 *        	A key of the answer in question.
	...
	protected function fill_answer_fields($answer, $questiondata, $key, $context) {
		...
		$answer->feedback = $this->import_or_save_files ( 
				$questiondata->feedback [$key], $context, 'question', 'answerfeedback', 
				$answer->id );
	...
	/**
	 * Return $answerextra, filling necessary fields for the extra answer fields
	 * table.
	 *
	 * The questions may want to overload it to save files or do other data
	 * processing.
	 *
	 * @param stdClass $answerextra
	 *        	Object to save data.
	 * @param object $questiondata
	 *        	This holds the information from the question editing form or import.
	 * @param int $key
	 ...
	 * @param object $context
	 *        	needed for working with files.
	 * @param array $extraanswerfields
	 *        	extra answer fields (without table name).
	 * @return $answer answerextra with filled data.
	 */
	protected function fill_extra_answer_fields($answerextra, $questiondata, $key, 
			$context, $extraanswerfields) {
		foreach ( $extraanswerfields as $field ) {
			// The $questiondata->$field[$key] won't work in PHP, break it down to two
			// strings of code.
	...
	public function save_hints($formdata, $withparts = false) {
		...
		foreach ( $oldhints as $oldhint ) {
			...
			$DB->delete_records ( 'question_hints', 
					array (
							'id' => $oldhint->id 
					) );
		}
	 ...
	 *        	whether to take into account clearwrong and shownumcorrect options.
	...
	protected function is_hint_empty_in_form_data($formdata, $number, $withparts) {
		if ($withparts) {
			return empty ( $formdata->hint [$number] ['text'] ) &&
					 empty ( $formdata->hintclearwrong [$number] ) &&
	 ...
	 *        	the data from the form.
	 * @param int $number
	 *        	number of hint to get options from.
	 * @param bool $withparts
	 *        	whether question have parts.
	 ...
	 *        	the $question->options object being built.
	 * @param object $formdata
	 *        	the data from the form.
	 * @param object $context
	 *        	the context the quetsion is being saved into.
	 * @param bool $withparts
	 *        	whether $options->shownumcorrect should be set.
	 */
	protected function save_combined_feedback_helper($options, $formdata, $context, 
			$withparts = false) {
		$options->correctfeedback = $this->import_or_save_files ( 
				$formdata->correctfeedback, $context, 'question', 'correctfeedback', 
				$formdata->id );
		$options->correctfeedbackformat = $formdata->correctfeedback ['format'];
		
		$options->partiallycorrectfeedback = $this->import_or_save_files ( 
				$formdata->partiallycorrectfeedback, $context, 'question', 
				'partiallycorrectfeedback', $formdata->id );
		...
		$options->incorrectfeedback = $this->import_or_save_files ( 
				$formdata->incorrectfeedback, $context, 'question', 
				'incorrectfeedback', $formdata->id );
	 ...
	 * @param object $question
	 *        	The question object for the question. This object
	...
	public function get_question_options($question) {
		...
		if (is_array ( $extraquestionfields )) {
			...
			$extra_data = $DB->get_record ( $question_extension_table, 
					array (
							$this->questionid_column_name () => $question->id 
			...
			if ($extra_data) {
				...
				echo $OUTPUT->notification ( 
						'Failed to load question options from the table ' .
								 $question_extension_table . ' for questionid ' .
								 $question->id );
				return false;
			}
		}
		...
		if (is_array ( $extraanswerfields )) {
			...
			if (! $question->options->answers) {
				echo $OUTPUT->notification ( 
						'Failed to load question answers from the table ' .
								 $answerextensiontable . 'for questionid ' .
								 $question->id );
				return false;
			}
		} else {
	 ...
	 *
	 * @param object $questiondata
	 *        	the question data loaded from the database.
	 ...
	 * @param object $questiondata
	 *        	the question data loaded from the database.
	 * @return question_definition an instance of the appropriate question_definition
	 *         subclass.
	 ...
	 *        	the question data loaded from the database.
	 */
	protected function initialise_question_hints(question_definition $question, 
			$questiondata) {
	 ...
	 *
	 * @param object $hint
	 *        	the DB row from the question hints table.
	 ...
	 *        	the question data loaded from the database.
	 * @param bool $withparts
	 *        	whether to set the shownumcorrect field.
	 */
	 ...
	 *        	most qtypes assume that answers are
	 ...
	 *        	ingores answerformat. Pass false here to use answerformat. For
	 *        	example
	 ...
	 *
	 * @param object $answer
	 *        	the DB row from the question_answers table plus extra answer fields.
	 * @return question_answer
	 */
	protected function make_answer($answer) {
		return new question_answer ( $answer->id, $answer->answer, $answer->fraction, 
	 ...
	 * @param int $contextid
	 *        	the context this quesiotn belongs to.
	 */
	 ...
	 * @param object $question
	 *        	The question whose length is to be determined.
	 ...
	 *
	 * @param object $question        	
	 * @return number null a fraction estimating what the student would
	...
	/**
	 * Whether or not to break down question stats and response analysis, for a
	 * question defined by $questiondata.
	 *
	 * @param object $questiondata
	 *        	The full question definition data.
	 * @return bool
	 */
	 ...
	 *
	 * @param object $form
	 *        	The data submitted by the previous page.
	 *        	
	...
	/*
	 * Imports question from the Moodle XML format Imports question using information
	 * from extra_question_fields function If some of you fields contains id's you'll
		...
		foreach ( $answers as $answer ) {
			...
			if (is_array ( $extraanswersfields )) {
				foreach ( $extraanswersfields as $field ) {
					$qo->{$field} [$a_count] = $format->getpath ( $answer, 
							array (
									'#',
									$field,
									0,
	...
	/*
	 * Export question to the Moodle XML format Export question using information from
	 * extra_question_fields function If some of you fields contains id's you'll need
	 * to reimplement this
	 */
	...
	public function generate_test($name, $courseid = null) {
		...
		$newcategory = question_make_default_categories ( 
				array (
						$context 
				) );
	 ...
	 *
	 * @param array $field
	 *        	the data from the form (or from import). This will
	 *        	normally have come from the formslib editor element, so it will be
	 *        	an
	 *        	array with keys 'text', 'format' and 'itemid'. However, when we are
	 *        	importing, it will be an array with keys 'text', 'format' and
	 *        	'files'
	 * @param object $context
	 *        	the context the question is in.
	 * @param string $component
	 *        	indentifies the file area question.
	 * @param string $filearea
	 *        	indentifies the file area questiontext,
	 *        	generalfeedback, answerfeedback, etc.
	 * @param int $itemid
	 *        	identifies the file area.
	 *        	
	 * @return string the text for this field, after files have been processed.
	 */
	protected function import_or_save_files($field, $context, $component, $filearea, 
			$itemid) {
		if (! empty ( $field ['itemid'] )) {
			// This is the normal case. We are safing the questions editing form.
			return file_save_draft_area_files ( $field ['itemid'], $context->id, 
					$component, $filearea, $itemid, $this->fileoptions, 
					trim ( $field ['text'] ) );
	 ...
	 *        	the question being moved.
	 * @param int $oldcontextid
	 *        	the context it is moving from.
	 * @param int $newcontextid
	 *        	the context it is moving to.
	 */
	 ...
	 * @param int $newcontextid
	 *        	the context it is moving to.
	 * @param bool $answerstoo
	 *        	whether there is an 'answer' question area,
	 *        	as well as an 'answerfeedback' one. Default false.
	 */
	protected function move_files_in_answers($questionid, $oldcontextid, $newcontextid, 
			$answerstoo = false) {
		global $DB;
		$fs = get_file_storage ();
		...
		foreach ( $answerids as $answerid => $notused ) {
			...
			$fs->move_area_files_to_new_context ( $oldcontextid, $newcontextid, 
					'question', 'answerfeedback', $answerid );
		}
	}
	 ...
	 *        	as well as an 'answerfeedback' one. Default false.
	 */
	protected function delete_files_in_answers($questionid, $contextid, 
			$answerstoo = false) {
		global $DB;
		$fs = get_file_storage ();
		...
		foreach ( $answerids as $answerid => $notused ) {
			...
			$fs->delete_area_files ( $contextid, 'question', 'answerfeedback', 
					$answerid );
		}
	}
	...
	protected function delete_files_in_combined_feedback($questionid, $contextid) {
		...
		$fs->delete_area_files ( $contextid, 'question', 'correctfeedback', 
				$questionid );
		$fs->delete_area_files ( $contextid, 'question', 'partiallycorrectfeedback', 
				$questionid );
		$fs->delete_area_files ( $contextid, 'question', 'incorrectfeedback', 
				$questionid );
	}
...
class question_possible_response {
	 ...
	 *        	see the field descriptions above.
	...
	public static function no_response() {
		return new question_possible_response ( 
				get_string ( 'noresponse', 'question' ), 0 );
	}
}
