...
abstract class qtype_renderer extends plugin_renderer_base {
	 ...
	 *        	the question attempt to display.
	 ...
	 * screen-readers.
	 * This method returns the text to use for the heading above
	 ...
	 * This is feedback that varies according to
	...
	protected function hint(question_attempt $qa, question_hint $hint) {
		return html_writer::nonempty_tag ( 'div', 
				$qa->get_question ()->format_hint ( $hint, $qa ), 
				array (
						'class' => 'hint' 
				) );
	}
	 ...
	 * @return string HTML fragment.
	 */
	public function manual_comment(question_attempt $qa, 
			question_display_options $options) {
		return '';
	}
	 ...
	 *
	 * @param $qa the
	 *        	question attempt that will be displayed on the page.
	 * @return string HTML fragment.
	 */
	 ...
	 *        	grade on a scale 0..1.
	 * @param bool $selected
	 *        	whether to show a big or small icon. (Deprecated)
	 * @return string html fragment.
	 */
...
abstract class qtype_with_combined_feedback_renderer extends qtype_renderer {
	protected function combined_feedback(question_attempt $qa) {
		...
		if (! $state->is_finished ()) {
			...
			list ( $notused, $state ) = $qa->get_question ()->grade_response ( 
					$response );
		}
		...
		if ($question->$field) {
			$feedback .= $question->format_text ( $question->$field, 
					$question->$format, $qa, 'question', $field, $question->id );
		}
