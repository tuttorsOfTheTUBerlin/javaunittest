 ...
 * This defines the common fields that
...
abstract class question_edit_form extends question_wizard_form {
	 ...
	 * Be careful how you use this it is needed sometimes to set up the structure of
	 * the
	 ...
	 * override this method and remove the ones you don't want with
	 * $mform->removeElement().
	 */
	protected function definition() {
		...
		if (! isset ( $this->question->id )) {
			...
			// Editing question with permission to move from category or save as new
			// q.
			$currentgrp = array ();
			...
			if ($this->question->formoptions->canedit ||
					 $this->question->formoptions->cansaveasnew) {
				// Not move only form.
				$currentgrp [1] = $mform->createElement ( 'checkbox', 
						'usecurrentcat', '', 
		...
		$mform->addElement ( 'editor', 'questiontext', 
		...
		$mform->addElement ( 'text', 'defaultmark', 
		...
		if (! empty ( $this->question->id )) {
			...
			if (! empty ( $this->question->createdby )) {
				...
				$a->user = fullname ( 
						$DB->get_record ( 'user', 
								array (
										'id' => $this->question->createdby 
								) ) );
			} else {
				$a->time = get_string ( 'unknown', 'question' );
				$a->user = get_string ( 'unknown', 'question' );
			}
			$mform->addElement ( 'static', 'created', 
					get_string ( 'created', 'question' ), 
			...
			if (! empty ( $this->question->modifiedby )) {
				...
				$a->user = fullname ( 
						$DB->get_record ( 'user', 
								array (
										'id' => $this->question->modifiedby 
								) ) );
				$mform->addElement ( 'static', 'modified', 
						get_string ( 'modified', 'question' ), 
						get_string ( 'byandon', 'question', $a ) );
			}
		}
		...
		if ($this->can_preview ()) {
			...
			$buttonarray [] = $mform->createElement ( 'static', 'previewlink', '', 
					$previewlink );
		}
		
		$mform->addGroup ( $buttonarray, 'updatebuttonar', '', 
				array (
						' ' 
				), false );
	...
	protected function can_preview() {
		return empty ( $this->question->beingcopied ) && ! empty ( 
				$this->question->id ) && $this->question->formoptions->canedit;
	}
	 ...
	 *        	the form being built.
	 * @param $label the
	 *        	label to use for each option.
	 * @param $gradeoptions the
	 *        	possible grades for each answer.
	 * @param $repeatedoptions reference
	 *        	to array of repeated options to fill
	 * @param $answersoption reference
	 *        	to return the name of $question->options
	...
	protected function get_per_answer_fields($mform, $label, $gradeoptions, 
		...
		$repeated = array ();
		$answeroptions = array ();
		$answeroptions [] = $mform->createElement ( 'text', 'answer', $label, 
				array (
						'size' => 40 
				) );
		$answeroptions [] = $mform->createElement ( 'select', 'fraction', 
				get_string ( 'grade' ), $gradeoptions );
		$repeated [] = $mform->createElement ( 'group', 'answeroptions', $label, 
	 ...
	 * @param $gradeoptions the
	 *        	possible grades for each answer.
	 * @param $minoptions the
	 *        	minimum number of answer blanks to display.
	 *        	Default QUESTION_NUMANS_START.
	 * @param $addoptions the
	 *        	number of answer blanks to add. Default QUESTION_NUMANS_ADD.
	 */
	...
	protected function add_combined_feedback_fields($withshownumpartscorrect = false) {
		...
		foreach ( $fields as $feedbackname ) {
			...
			$element->setValue ( 
					array (
							'text' => get_string ( $feedbackname . 'default', 
									'question' ) 
					) );
	 ...
	 *        	whether this quesiton type uses the 'Show num parts correct' option
	 *        	on hints.
	 * @return array form field elements for one hint.
	 */
	protected function get_hint_fields($withclearwrong = false, 
		...
		$repeated [] = $mform->createElement ( 'editor', 'hint', 
				get_string ( 'hintn', 'question' ), array (
						'rows' => 5 
				), $this->editoroptions );
		...
		if ($withclearwrong) {
			$optionelements [] = $mform->createElement ( 'advcheckbox', 
					'hintclearwrong', get_string ( 'options', 'question' ), 
		...
		if (count ( $optionelements )) {
			$repeated [] = $mform->createElement ( 'group', 'hintoptions', 
					get_string ( 'hintnoptions', 'question' ), $optionelements, null, 
					false );
		}
	...
	protected function add_interactive_settings($withclearwrong = false, 
		...
		if (! empty ( $this->question->penalty ) &&
				 ! in_array ( $this->question->penalty, $penalties )) {
	...
	public function set_data($question) {
		question_bank::get_qtype ( $question->qtype )->set_default_options ( 
				$question );
		...
		$question->generalfeedback ['format'] = empty ( 
				$question->generalfeedbackformat ) ? editors_get_preferred_format () : $question->generalfeedbackformat;
	...
	protected function data_preprocessing_answers($question, $withanswerfiles = false) {
		...
		foreach ( $question->options->answers as $answer ) {
            ...
            unset// to unset ->_defaultValues['fraction[0]'].
			
			null ( $this->_form->_defaultValues ["fraction[$key]"] );
			
			// Prepare the feedback editor to display files in draft area.
            $draftitemid// Prepare the feedback editor to display files in draft area.
			null = 
			file_get_submitted_draft_itemid ( 'feedback[' . $key . ']' );
			$question->feedback [$key] ['text'] = file_prepare_draft_area ( 
					$draftitemid, 					// Draftid
                $this// Draftid
					null->context->id, 					// context
                'question'// context // component
                'answerfeedback'// component // filarea
                // filarea
					null! empty ( $answer->id ) ? ( int ) $answer->id : null,  // itemid
                $this// itemid
					null->fileoptions, 					// options
                $answer// options
					null->feedback )			// text.
            )// text.
        ...
        $extraanswerfields// Now process extra answer fields.
		null = 
		question_bank::get_qtype ( $question->qtype )->extra_answer_fields ();
		if (is_array ( $extraanswerfields )) {
			// Omit table name.
			// Omit table name.
			nullarray_shift ( $extraanswerfields );
			$question = $this->data_preprocessing_extra_answer_fields ( $question, 
					$extraanswerfields );
		}
	 ...
	 * @return object $question the modified data.
	 */
	protected function data_preprocessing_extra_answer_fields($question, 
			$extraanswerfields) {
        ...
        $extrafieldsdata// Index in $extrafieldsdata is an extra answer field name, value - array of it's data for each answer.
		null = 
		array ();
		// First, prepare an array if empty arrays for each extra answer fields data.
        foreach// First, prepare an array if empty arrays for each extra answer fields data.
		null ( $extraanswerfields as $field ) 
		{
			$extrafieldsdata [$field] = array ();
		}
		
		// Fill arrays with data from $question->options->answers.
        $key// Fill arrays with data from $question->options->answers.
		null = 
		0;
		foreach ( $question->options->answers as $answer ) {
			foreach ( $extraanswerfields as $field ) {
				// See hack comment in {@link data_preprocessing_answers()}.
                unset// See hack comment in {@link data_preprocessing_answers()}.
				
				null ( $this->_form->_defaultValues ["$field[$key]"] );
				$extrafieldsdata [$field] [$key] = $this->data_preprocessing_extra_answer_field ( 
						$answer, $field );
			}
			$key ++;
		}
        ...
        foreach// Set this data in the $question object.
		null ( $extraanswerfields as $field ) 
		{
	 ...
	 *
	 * @param object $answer
	 *        	an answer object to get extra field from.
	 * @param string $field
	 *        	extra answer field name.
	...
	protected function data_preprocessing_combined_feedback($question, 
		...
		foreach ( $fields as $feedbackname ) {
			...
			$feedback = array ();
			$feedback ['text'] = file_prepare_draft_area ( $draftid,  // Draftid
                $this// Draftid
					null->context->id, 					// context
                'question'// context // component
                $feedbackname// component // filarea
                // filarea
					null! empty ( $question->id ) ? ( int ) $question->id : null,  // itemid
                $this// itemid
					null->fileoptions, 					// options
                $question// options
					null->options->$feedbackname )			// text.
            )// text.
	...
	protected function data_preprocessing_hints($question, $withclearwrong = false, 
		...
		foreach ( $question->hints as $hint ) {
            ...
            $draftitemid// Prepare feedback editor to display files in draft area.
			null = 
			file_get_submitted_draft_itemid ( 'hint[' . $key . ']' );
			$question->hint [$key] ['text'] = file_prepare_draft_area ( $draftitemid,  // Draftid
                $this// Draftid
					null->context->id, 					// context
                'question'// context // component
                'hint'// component // filarea
                // filarea
					null! empty ( $hint->id ) ? ( int ) $hint->id : null,  // itemid
                $this// itemid
					null->fileoptions, 					// options
                $hint// options
					null->hint )			// text.
	...
	public function validation($fromform, $files) {
		...
		if (empty ( $fromform ['makecopy'] ) && isset ( $this->question->id ) && ($this->question->formoptions->canedit ||
				 $this->question->formoptions->cansaveasnew) &&
		...
		// Default mark.
        if// Default mark.
		null (array_key_exists ( 'defaultmark', $fromform ) &&
				 $fromform ['defaultmark'] < 0) 
		{
			$errors ['defaultmark'] = get_string ( 'defaultmarkmustbepositive', 
					'question' );
		}
	...
	protected function get_non_collabsible_editor_options() {
		debugging ( 
				'get_non_collabsible_editor_options() is deprecated, use $this->editoroptions instead.', 
				DEBUG_DEVELOPER );
